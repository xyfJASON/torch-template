...
from utils.scheduler import ConstantWarmupLR, LinearWarmupLR, CosineWarmupLR
...
def main():
    ...
    # BUILD MODEL (WRAPPED WITH DDP)
    model = UNet(**conf.model).to(device)
    ...
    # BUILD EMA MODEL
    ema = EMA(model, decay=float(conf.ema.decay))
    logger.info(f"EMA decay: {ema.decay}")
        ...
        if global_step % conf.sampling.every_steps == 0 or global_step == 1:
            ...
            with torch.no_grad():
                ...
                for t, t_prev in zip(timesteps[:-1], timesteps[1:]):
                    v_pred = sampling_model(samples, t.repeat((num_samples, )))
